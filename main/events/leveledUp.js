...
export default class LeveledUpEvent {
  ...
  execute = async ( {message, level} ) => {
    ...
    await message.channel.send(lvlMsg)
      .then(msg => {
    ...
    try {await author.roles.add(roleObjs)}
    catch ( e ) {this.bot.logger.error( e ); return;}

    // Send a Message about the level up and gained roles.
    const msg = `\`${author.displayName} gained the following title(s):  ${roleObjs.map(r => r.name).join(', ')} \``
